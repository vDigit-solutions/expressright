...
import com.express.dao.videos.VideosDaoImpl;
...
public class UploadDaoImpl extends JdbcDaoSupport implements UploadDao {
	...
	@Override
	public Number persistFileInfo(final String filrPath,final String contentType,
			final String description,final Integer theme,final String name2,final String thumbnail,
			final Long session_id ,final Future<Double> duration,final String mp4Address) {
		Number videoId = null;
		try {
			...
			final PreparedStatementSetter paramsSetter = new PreparedStatementSetter() {
				...
				public void setValues(PreparedStatement ps) throws SQLException {
					try {
						...
						//,user_id,video_added_datetime ,video_updated_datetime ,video_status ,thumbnail , video_duration 
	...
	private Long getUseridBySession(Long session_id) {
		...
		return getJdbcTemplate().queryForObject( "SELECT user_id FROM user_login_history WHERE id = ? ", Long.class, session_id);
	}
