...
public interface VideosDao {
	...
	String queryVideosByTheme = "SELECT * FROM ( SELECT ROW_NUMBER() OVER( ORDER BY video_id ) as slNo,video_id ,video_name , file_mime_type , theme_type , video_description , video_duration , video_uploaded_time , file_url , thumbnail_url , user_name , theme_id , theme_name , view_count, like_count FROM v_online_videos ov JOIN index_themes it ON it.theme_id = ov.theme_type WHERE  theme_id = ? ) AS tbl  WHERE slNo BETWEEN ? AND ? ";
	String queryVideosSearch = "SELECT * FROM ( SELECT ROW_NUMBER() OVER( ORDER BY video_id ) as slNo,video_id ,video_name , file_mime_type , theme_type , video_description , video_duration , video_uploaded_time , file_url , thumbnail_url , user_name , theme_id , theme_name , view_count, like_count FROM v_online_videos ov JOIN index_themes it ON it.theme_id = ov.theme_type  WHERE slNo BETWEEN ? AND ? ";
	String queryDeleteVideo = "DELETE FROM online_video WHERE user_id = (SELECT user_id FROM user_login_history WHERE id = :id ) AND video_id IN (:video_ids)";
	...
	List<Map<String, Object>> getAllVideosbyTheme(Long theme_id, Integer pageNo, Integer pageSize);
	...
	Collection<Map<String, Object>> getVideosbyVideoIds(List<Long> videoIds);
