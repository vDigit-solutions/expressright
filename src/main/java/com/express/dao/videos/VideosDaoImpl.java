...
public class VideosDaoImpl extends JdbcDaoSupport implements VideosDao{
	...
	public VideosDaoImpl(JdbcTemplate jdbcTemplate){
		...
		@Override
	public List<Map<String, Object>> getAllVideos(Integer pageNo,
			Integer pageSize) {
		...
		//String selectQuery = queryVideos + " ORDER BY video_id OFFSET "+offset+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY;";
	...
	public List<Map<String, Object>> getAllVideosbyTheme(Long theme_id,
		...
		//String selectQuery = queryVideosByTheme +" OFFSET  " + offset+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY;";
		result = getJdbcTemplate().query(selectQuery, new Object[]{theme_id,offset , offset+pageSize},new ColumnMapRowMapper());
		return result;
	}
	...
	public List<Map<String, Object>> getAllVideosbyTheme(Long theme_id) {
		...
		result = getJdbcTemplate().query("SELECT * FROM v_online_videos_by_theme WHERE theme_type = ? ", new Object[]{theme_id},new ColumnMapRowMapper());
		return result;
	}
	...
	public Map<String, Object> getVideobyVideoId(Long videoId) {
		...
		result = getJdbcTemplate().queryForMap(" SELECT * FROM v_online_videos WHERE video_id = ? ", new Object[]{videoId});
		return result;
	}
	...
	public Map<String, Object> getVideoDetailbyVideoId(Long videoId) {
		...
		result = getJdbcTemplate().queryForMap(" SELECT theme_type , thumbnail , file_path FROM online_video WHERE video_id = ? ", new Object[]{videoId});
		return result;
	}
	...
	private List<Map<String, Object>> searchVideosByTheme(
			Long theme, Integer pageNo, Integer pageSize) {
	...
	public void playBackStarted(Long video_id, Long user_session_id, Integer apptype, Integer device_type) {
		...
		Long user_id = jt.queryForObject("SELECT user_id FROM user_login_history WHERE id = ? ",new Object[]{user_session_id}, Long.class);
		jt.update("UPDATE ov SET ov.view_count = ov.view_count+1 FROM online_video ov WHERE ov.video_id = ?",video_id);
		jt.update("INSERT INTO history_viewed_log(video_id , user_id , app_type , view_datetime) values( ? , ? , ? , ?)",new Object[]{video_id , user_id , apptype , new Date()});
