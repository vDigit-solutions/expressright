...
public class VideoSearchDaoImpl extends JdbcDaoSupport implements VideoSearchDao {
	...
	public List<Map<String, Object>> searchVideos(String query, Integer theme) {
		...
		if (theme >= 0) {
			result = getJdbcTemplate().query(this.queryVideosSearch,
					new Object[] { "%" + query + "%", "%" + query + "%", theme }, new ColumnMapRowMapper());
		} else {
			result = getJdbcTemplate().query(this.queryWithoutTheme,
					new Object[] { "%" + query + "%", "%" + query + "%" }, new ColumnMapRowMapper());
	...
	@Override
	public Collection<Map<String, Object>> searchVideos(String query, Long theme, Integer pageNo, Integer pageSize) {
		...
		if (theme >= 0) {
			...
			List<Map<String, Object>> themes = getJdbcTemplate().queryForList(this.queryTheme,
	...
	private List<Map<String, Object>> searchVideosByTheme(String query, Long theme, Integer pageNo, Integer pageSize) {
		// TODO Auto-generated method stub
		int offset = (pageNo - 1) * pageSize + 1;
		if (query != null && query.length() > 0) {
			return getJdbcTemplate().queryForList(this.queryVideosSearch,
					new Object[] { query, query, theme, offset, offset + pageSize });
		} else {
			return getJdbcTemplate().queryForList(this.queryVideosByTheme,
					new Object[] { theme, offset, offset + pageSize });
